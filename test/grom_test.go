...
import (
	...
	gorm "github.com/wrpota/go-echo/pkg/grom"
)

//  gorm v2  操作数据库单元测试
// 测试本篇首先保证 config/gorm.yml 文件配置正确，相关配置项 IsInitGolobalGormMysql = 1
...
// 由于配置文件（config/gorm.yml）只提供了一份mysql连接，无法满足需求，这时您可以通过自定义参数直接连接任意数据库，获取一个数据库句柄，供业务使用
func TestCustomeParamsConnMysql(t *testing.T) {
	...
	if gormDbMysql, err := gorm.GetSqlDriver("mysql", 0, confPrams); err == nil {
...
// 请在配置项 config > gorm.yml 中，sqlserver 部分，正确配置数据库参数
...
//  PostgreSql 数据库测试
// 请在配置项 config > gorm.yml 中，PostgreSql 部分，正确配置数据库参数。
